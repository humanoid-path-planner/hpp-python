...
struct Builder {
  ...
  template <typename T>
  static void add_to_container(Container<T>& c, const std::string& key,
                               PyObject* callable) {
...
struct exposeContainer {
  static void run(const char* name) {
    ...
    class_<C_t>(name, no_init) PYHPP_DEFINE_METHOD(
        C_t, erase) PYHPP_DEFINE_METHOD(C_t, clear)
...
void exposeProblemSolver() {
  ...
  class_<ProblemSolver>("ProblemSolver", no_init)
      ...
      .def("problem",
           ...
           return_internal_reference<>())
      // .def ("initConfig", static_cast<const ConfigurationPtr_t&
      // (ProblemSolver::*) () const>(&ProblemSolver::initConfig),
      // return_internal_reference<>()) .def ("initConfig", static_cast<void
      // (ProblemSolver::*) (const ConfigurationPtr_t&)
      // >(&ProblemSolver::initConfig))
      .def("initConfig", &PSWrapper::setInitConfig)
      .def("initConfig", &PSWrapper::getInitConfig) PYHPP_DEFINE_METHOD2(
          PSWrapper, goalConfigs, DocClassMethod(goalConfigs))
          PYHPP_DEFINE_METHOD2(PSWrapper, addGoalConfig,
                  ...
                  PYHPP_DEFINE_METHOD2(ProblemSolver, resetProblem,
                      ...
                      PYHPP_DEFINE_METHOD2(ProblemSolver, resetRoadmap,
                          ...
                          PYHPP_DEFINE_METHOD2(
                              ...
                              PYHPP_DEFINE_METHOD2(
                                  ProblemSolver, prepareSolveStepByStep,
      ...
      PYHPP_DEFINE_METHOD2(ProblemSolver, addPath, DocClassMethod(addPath))
          PYHPP_DEFINE_METHOD2(ProblemSolver, erasePath,
                               DocClassMethod(erasePath))
              PYHPP_DEFINE_METHOD_INTERNAL_REF(ProblemSolver, paths)
                  ...
                  PYHPP_DEFINE_METHOD2(ProblemSolver, createRobot,
                      ...
                      PYHPP_DEFINE_GETTER_SETTER_INTERNAL_REF(
                          ProblemSolver, robot, const DevicePtr_t&)
      .def("addObstacle",
           static_cast<void (ProblemSolver::*)(const DevicePtr_t&, bool, bool)>(
               &ProblemSolver::addObstacle),
           DocClassMethod(addObstacle))

          PYHPP_PROBLEMSOLVER_SELECT_TYPE(
              robot) PYHPP_PROBLEMSOLVER_SELECT_TYPE(pathPlanner)
      ...
      PYHPP_PROBLEMSOLVER_CONTAINER(pathPlanners)
          PYHPP_PROBLEMSOLVER_CONTAINER(pathOptimizers)
              ...
              PYHPP_DEFINE_GETTER_SETTER(ProblemSolver, maxIterProjection,
                                         size_type)
                  PYHPP_DEFINE_GETTER_SETTER(ProblemSolver, maxIterPathPlanning,
                                             size_type)
