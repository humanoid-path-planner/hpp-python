...
namespace pyhpp {
  namespace core {
    ...
    struct CWrapper {
      ...
      static bool isSatisfied2 (Constraint& cs, const vector_t& q, eigenpy::Ref<vector_t>& error)
      {
        vector_t _err (error);
    ...
    void exposeConstraint ()
    {
      ...
      class_ <ConstraintSet, ConstraintSetPtr_t, boost::noncopyable, bases<Constraint> >
        ("ConstraintSet", no_init)
        PYHPP_DEFINE_METHOD (ConstraintSet, addConstraint)
        PYHPP_DEFINE_METHOD (ConstraintSet, configProjector)
        ;

      class_ <ConfigProjector, ConfigProjectorPtr_t, boost::noncopyable, bases<Constraint> >
        ("ConfigProjector", no_init)
        PYHPP_DEFINE_METHOD_INTERNAL_REF (ConfigProjector, solver)

        .def ("add", static_cast <void (ConfigProjector::*) (const LockedJointPtr_t&)> (&ConfigProjector::add))
        .def ("add", static_cast <bool (ConfigProjector::*) (const NumericalConstraintPtr_t&, const segments_t&, const std::size_t)>
            (&ConfigProjector::add), CP_add_overloads())
        ...
        .def ("errorThreshold", static_cast <void (ConfigProjector::*) (const value_type&)> (&ConfigProjector::errorThreshold))
        .def ("errorThreshold", static_cast <value_type (ConfigProjector::*) () const     > (&ConfigProjector::errorThreshold))
        PYHPP_DEFINE_METHOD (ConfigProjector, residualError)
        .def ("sigma", &ConfigProjector::sigma, return_value_policy<return_by_value>())
