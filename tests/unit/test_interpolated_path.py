...
class TestInterpolatedPath(unittest.TestCase):
    def test_preserves_points_through_vector(self):
        ...
        self.assertIsInstance(retrieved, InterpolatedPath)
        points = retrieved.interpolationPoints()
        self.assertEqual([t for t, _ in points], [0.0, 1.0, 2.0])
        np.testing.assert_allclose(points[1][1], q1)
        for start, end in ((0.0, 1.0), (1.0, 2.0)):
            piece = retrieved.extract(start, end)
            self.assertEqual(len(piece.interpolationPoints()), 2)
        points[1][1][0] = 99.0
