...
r1_pose = SE3(Quaternion(0, 0, 0, 1), np.array([0.25, 0, 0]))
...
for i in range(nSphere):
    ...
    urdf.loadModel(
        robot, 0, f"sphere{i}", "freeflyer", sphere_urdf, sphere_srdf, sphere_pose
    )
...
for i in range(nSphere):
    ...
    constraints[placementName + "/complement"] = Implicit(
        pc_complement, cts_complement, [True, True, True]
    )
...
y = 0.04
while i < nSphere:
    q0_spheres.append([-0.1 * (i / 2), -0.12 + y, 0.025, 0, 0, 0, 1])
    i += 1
    y = -y
...
y = -0.04
while i < nCylinder:
    q0_cylinders.append([0.45 + 0.1 * (i / 2), -0.12 + y, 0.025, 0, 0, 0, 1])
    i += 1
    y = -y
...
if not args.bigGraph:
    ...
    sm = SecurityMargins(
        problem,
        factory,
        ["r0", "r1", "sphere0", "sphere1", "cylinder0", "cylinder1"],
        robot,
    )
