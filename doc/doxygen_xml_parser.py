...
class Index:
    ...
    def _getCompound (self, kind, name):
        return self.tree.xpath("/doxygenindex/compound[@kind='" + kind + "' and name='" + name + "']")[0]
...
class ClassDoc:
    ...
    def _getMember (self, sectionKind, memberDefKind, name):
        ...
        try:
            return self.compound.xpath ("sectiondef[re:test(@kind,'" + sectionKind
                    + "')]/memberdef[@kind='" + memberDefKind
                    + "' and name='" + name + "']",
                    namespaces={'re': "http://exslt.org/regular-expressions"}
                    )[0]
        except IndexError as e:
            msg = "Error: Could not find member (" + sectionKind + ") " + name + " of class " + self.classname
    ...
    def getClassMethodDoc (self, methodname):
        ...
        if member.get("static") == "no": args = ["self",]
        else                           : args = []
        args += [ el.text.strip() for el in member.xpath("param/declname") ]
        for parameters in dd.xpath("para/parameterlist/parameteritem"):
            pargs = [ el.text.strip() for el in parameters.find("parameternamelist").findall("parametername")]
            pns = " ".join (pargs)
